...
def calculate_LEs_QR_jax_with_U(esn_jacobian_jax, reservoir, N_transient, dt, norm_step=1, target_dim=None, randomseed=0, U=None):
    ...
    idx, U, (Q_tracked, R_tracked, LEs_tracked) = var_equation(N - N_transient, idx, U)
    ...
    return LEs, Q_tracked, R_tracked, U


def calculate_LEs_QR_jax(esn_jacobian_jax, reservoir, N_transient, dt, norm_step=1, target_dim=None, randomseed=0):
    """Calculate the Lyapunov exponents
    Args:
        reservoir: state trajectory
        target_dim: dimension of the target system, valid for ESN, e.g. <3 for Lorenz 64
    ...
    def var_equation(n, idx, U):
        def body(carry, _):
            ...
            jac = jnp.multiply(esn_jacobian_jax, 1.0 - reservoir[idx] ** 2)
            U = jnp.matmul(jac, U)
            U, R = jax.scipy.linalg.qr(U[:, :target_dim], mode="economic")
            idx = idx + 1
            return (idx, U), (U, R, jnp.abs(jnp.diag(R[:target_dim, :target_dim])))
    ...
    # idx, U, (U_tracked, R_tracked, LEs_tracked) = var_equation(N, 0, U)
    # LEs = jnp.cumsum(jnp.log(LEs_tracked[N_transient:]), axis=0) / jnp.tile(T[:], (target_dim, 1)).T
    return LEs, Q_tracked, R_tracked
...
def calculate_LEs_jax(esn_jacobian_jax, reservoir, N_transient, dt, norm_step=1, target_dim=None, randomseed=0):
    ...
    def var_equation(n, idx, U):
        def body(carry, _):
            ...
            U_new, R = jax.scipy.linalg.qr(U_new[:, :target_dim], mode="economic")
            idx = idx + 1
            return (idx, U_new), jnp.abs(jnp.diag(R[:target_dim, :target_dim]))

        (idx, U), LE_all = jax.lax.scan(body, (idx, U), xs=None, length=n)
        return idx, U, LE_all

    idx, U, _ = var_equation(N_transient, 0, U)
    idx, U, LEs_tracked = var_equation(N - N_transient, idx, U)
